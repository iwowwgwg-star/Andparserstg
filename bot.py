...
def post_to_vk_with_media(text, media_urls):
    try:
        ...
        # ЖЕСТКОЕ ПРАВИЛО: Если нет ни одной успешно загруженной фотографии — отменяем публикацию
        if not attachments:
            print("ПРОПУСК: В посте нет фотографий, публикация отменена.", flush=True)
            return False

        # Публикуем строго от имени сообщества без подписи автора
        vk.wall.post(
        ...
        print("SUCCESS: Пост с фото успешно опубликован от имени сообщества!", flush=True)
        time.sleep(3)
        return True
    except Exception as e:
            ...
            if recent_posts:
                ...
                for post in recent_posts:
                    ...
                    if published:
                        ...
                        # Если пост без фото, добавляем в processed_ids, чтобы больше не пытаться его проверять
                        processed_ids.add(post["id"])
            else:
