...
def bot_main_loop():
    print("Бот запущен. Проверка постов за последние 2 недели...", flush=True)
    two_weeks_ago = datetime.now() - timedelta(days=14)

    # 1. Загружаем архив за 2 недели при первом старте
    ...
    print(f"Архив обработан. Переходим в режим мониторинга новых постов...", flush=True)

    # 2. Бесконечный цикл проверки новых постов
    while True:
        try:
...
# СНАЧАЛА запускаем веб-сервер в фоновом потоке, чтобы Render сразу увидел порт и статус Live
...
# А ТЕПЕРЬ запускаем самого бота (он спокойно будет выгружать посты, не мешая Render)
bot_thread = threading.Thread(target=bot_main_loop)
bot_thread.daemon = True
bot_thread.start()

# Главный поток просто висит, удерживая приложение живым
while True:
    time.sleep(3600)
