...
def post_to_vk_with_media(text, media_urls):
    try:
        ...
        response = vk_session.method('wall.post', {
        ...
        print(f"УСПЕХ! Ссылка на пост в группе -> https://vk.com/wall-{VK_GROUP_ID}_{post_id}", flush=True)
        time.sleep(3)
        return True
    except Exception as e:
        print(f"ОШИБКА API ВК: {e}", flush=True)
        return False
