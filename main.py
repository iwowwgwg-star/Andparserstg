...
def download_and_upload_video_yt_dlp(post_url):
    """Скачивает видео с помощью yt-dlp по ссылке на пост Телеграма и заливает в ВК"""
    try:
        if os.path.exists(TEMP_VIDEO_PATH):
            os.remove(TEMP_VIDEO_PATH)

        print(f"Пытаемся скачать видео через yt-dlp из поста: {post_url}")
        ydl_opts = {
            'format': 'best',
        ...
        if not os.path.exists(TEMP_VIDEO_PATH) or os.path.getsize(TEMP_VIDEO_PATH) < 50000:
            print("yt-dlp не смог найти или скачать видео из этого поста (возможно, его там нет).")
            return None

        print(f"Видео успешно скачано локально. Размер: {os.path.getsize(TEMP_VIDEO_PATH)} байт. Загружаем в ВК...")
        ...
        with open(TEMP_VIDEO_PATH, 'rb') as f:
            ...
            upload_res = requests.post(upload_url, files=files, timeout=120).json()
        ...
        print(f"Ошибка при скачивании/загрузке видео через yt-dlp: {e}")
    finally:
        if os.path.exists(TEMP_VIDEO_PATH):
            os.remove(TEMP_VIDEO_PATH)
    return None
...
def fetch_latest_telegram_post():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if not link:
                return None
            post_id = link['href'] # Пример: https://t.me/topor/52080
            ...
            # Проверяем, есть ли в посте вообще признаки видео
            has_video = False
            if msg.find('video') or msg.find('source') or msg.find(class_=['tgme_widget_message_video_player', 'tgme_widget_message_video']):
                has_video = True

            return {"id": post_id, "text": text, "media": media, "has_video": has_video}
    except Exception as e:
...
def main():
    ...
    if post_id in processed:
        print(f"Пост {post_id} уже есть в базе. Новых нет.")
        return

    if not latest_post["media"] and not latest_post["has_video"] and not latest_post["text"]:
        print("Пост пустой, пропускаем.")
        return

    print(f"Новый пост найден: {post_id}. Начинаем публикацию...")

    # Фиксируем пост в базе сразу, чтобы не слать дубликаты
    ...
    # 2. Если в посте есть видео — скачиваем его через yt-dlp по ссылке на пост
    if latest_post["has_video"]:
        att = download_and_upload_video_yt_dlp(post_id)
        if att:
            attachments.append(att)
    ...
    try:
        ...
        print("Пост успешно опубликован в ВК со всеми медиафайлами!")
