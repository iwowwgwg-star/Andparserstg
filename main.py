...
def bot_loop():
    ...
    print("Бот запущен в фоне и проверяет посты...")
    
    current_posts = fetch_telegram_posts()
    
    # Разворачиваем, чтобы старые шли первыми
    current_posts.reverse()
    
    print(f"Найдено постов на странице: {len(current_posts)}")
    
    # Публикуем последние 10 постов со страницы при самом первом старте
    for post in current_posts[-10:]:
        print(f"Публикуем начальный пост...")
        send_to_vk(post["text"], post["media"])
        processed_posts.add(post["id"])
        time.sleep(2) # Пауза между постами, чтобы ВК не блокировал за спам

    # Заносим остальные в базу, чтобы не дублировать
    for post in current_posts:
        processed_posts.add(post["id"])

    print("Инициализация завершена. Переходим в режим мониторинга новых постов.")
