...
def fetch_latest_telegram_post():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # Проходим по последним сообщениям, ищем первое подходящее (без видео, но с фото)
            for msg in reversed(messages[-5:]):
                ...
                # Проверяем наличие видео в посте (любые признаки)
                has_video = bool(
                    msg.find('video') or 
                    msg.find('source') or 
                ...
                if has_video:
                    print(f"Пост {post_id} содержит видео — пропускаем его.")
                    continue
                ...
                # Если картинок нет — тоже пропускаем (нужны строго посты с картинками)
                if not media:
                    continue
...
def main():
    ...
    if not latest_post:
        print("Подходящих постов с картинками (без видео) не найдено.")
        return
    ...
    print(f"Найден подходящий пост с картинками: {post_id}. Публикуем...")

    # Сразу добавляем в обработанные, чтобы не спамить дубликатами
    processed.add(post_id)
    save_and_push_processed(processed)

    attachments = []
    
    # Загружаем все найденные фото
    ...
    try:
        ...
        print("Пост с картинками успешно опубликован в ВК!")
