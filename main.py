...
def upload_media_to_wall(media_urls, video_urls, text=""):
    ...
    # 2. Загружаем видео с описанием на стену и в Клипы
    for v_url in video_urls[:2]:
        try:
            print(f"Скачиваем видео для загрузки в ВК и Клипы...")
            ...
            if v_resp.status_code == 200:
                ...
                # Загружаем видео на стену с сохранением описания (name)
                ...
                # Дублируем это же видео в VK Клипы от имени сообщества
                try:
                    print(f"Публикуем видео также в VK Клипы...")
                    ...
                    print(f"Не удалось отправить в Клипы (возможно, требуются особые права): {clip_err}")
...
def send_to_vk(text, media_urls, video_urls):
    if not media_urls and not video_urls:
        return False
    try:
        # Передаем текст внутрь загрузчика, чтобы видео получило описание
        ...
        print("Пост (фото + видео + текст) успешно опубликован в VK!")
        return True
    except Exception as e:
...
def main():
    ...
    for post in posts:
        ...
        if clean_msg and clean_msg not in vk_texts:
            print("Найден комплексный пост (текст + медиа/видео), публикуем...")
            success = send_to_vk(clean_msg, post["media"], post["videos"])
            if success:
                vk_texts.add(clean_msg)
                published_count += 1
                time.sleep(8) # Увеличили паузу, так как загрузка видео тяжелее
        else:
            print("Пост пропущен (дубль, пустой или рекламный).")
