...
def get_all_tg_posts():
    ...
    # Ставим честный User-Agent под мобильный браузер, чтобы ТГ отдавал страницу
    headers = {
    ...
    try:
        ...
        print(f"Статус ответа Telegram для канала {TG_CHANNEL}: {response.status_code}")
        ...
        print(f"Найдено блоков сообщений на странице: {len(messages)}")
