...
def get_vk_wall_texts():
    """Получает последние тексты постов со стены ВК для сравнения"""
    url = "https://api.vk.com/method/wall.get"
    params = {
        "owner_id": GROUP_ID,
        "count": 20,  # Проверяем последние 20 постов на стене
    ...
    try:
        ...
        if "response" in res and "items" in res["response"]:
            # Очищаем и приводим к нижнему регистру для точного сравнения
...
if __name__ == "__main__":
    ...
    vk_texts = get_vk_wall_texts()  # Загружаем посты с ВК для проверки
    ...
    for post in posts:
        ...
        # 1. Проверка по локальной истории (ID поста)
        if tg_id in history:
            continue
            
        # 2. Проверка по текстам в ВК (сравнение по словам / содержимому)
        is_duplicate = False
        if post_text_clean:
            for vk_t in vk_texts:
                # Если первые 50 символов или весь текст совпадают
                if post_text_clean in vk_t or vk_t in post_text_clean:
                    is_duplicate = True
                    break
        
        if is_duplicate:
            print(f"Пост {tg_id} уже есть в VK (найдено совпадение по тексту), пропускаем.")
            history.append(tg_id)  # Заносим в историю, чтобы больше не проверять
