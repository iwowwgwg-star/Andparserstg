...
def get_vk_wall_texts():
    """Получает тексты последних постов со стены ВК для жесткой защиты от дублей"""
    try:
        url = "https://api.vk.com/method/wall.get"
        params = {
            "owner_id": -GROUP_ID,
            "count": 50,  период проверки стены
...
def main():
    ...
    for post in posts:
        # 1. Проверка по локальному файлу
        if post["id"] in processed:
            continue

        # Отмечаем сразу, чтобы больше не возвращаться
        ...
        # 2. Жёсткая проверка по текстам уже опубликованных постов в ВК (защита от дублей)
        post_text_lower = post["text"].lower()
        if post_text_lower and len(post_text_lower) > 15:  # сравниваем только если есть содержательный текст
            is_duplicate = any(post_text_lower in vk_t or vk_t in post_text_lower for vk_t in vk_texts if vk_t)
            if is_duplicate:
                print(f"Пост {post['id']} уже найден на стене ВК, пропускаем дубль.")
                continue
